...
use halo2_proofs::{
    ...
    plonk::{
        create_proof, keygen_pk, keygen_vk, verify_proof, Circuit as PlonkCircuit,
...
struct AesKeySchedulingCircuit {
    key: [u8; 16],
}

impl PlonkCircuit<Fr> for AesKeySchedulingCircuit {
...
impl CircuitExt<Fr> for AesKeySchedulingCircuit {
    fn instances(&self) -> Vec<Vec<Fr>> {
        vec![]
    }

    fn rand(_k: usize, mut rng: impl rand::prelude::RngCore) -> Self
    where
        Self: Sized,
    {
        let mut key = [0u8; 16];
...
fn main() {
    ...
    bench_plonkish_backend::<HyperPlonk, AesKeySchedulingCircuit>(System::HyperPlonk, k);
    ...
    // bench_plonkish_backend::<UniHyperPlonk, AesKeySchedulingCircuit>(System::UniHyperPlonk, k)
}
